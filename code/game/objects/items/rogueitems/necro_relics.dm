...
/obj/item/necro_relics/necro_crystal/attack_self(mob/living/user)
    ...
    var/task = input(user, "What task do you wish to assign your Greater Skeleton?", "Assign Task") as null|text
    if(!task)
    ...
    var/list/candidates = pollGhostCandidates("The veil splits! A hand reaches forth! Do you want to serve [necro_name] as their Greater Skeleton? \n WHAT IS MY PURPOSE? [task]", ROLE_NECRO_SKELETON, null, null, 10 SECONDS, POLL_IGNORE_NECROMANCER_SKELETON)
