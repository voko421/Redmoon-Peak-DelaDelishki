...
/obj/structure/far_travel/MouseDrop_T(atom/dropping, mob/user)
	...
	user.visible_message("<span class='warning'>[user] [departing_mob == user ? "is trying to depart from [SSmapping.map_adjustment?.realm_name]!" : "is trying to send [departing_mob] away!"]</span>", "<span class='notice'>You [departing_mob == user ? "are trying to depart from [SSmapping.map_adjustment?.realm_name]." : "are trying to send [departing_mob] away."]</span>")
	in_use = TRUE
